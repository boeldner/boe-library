...
if (redirectUrl) {
    ...
    // Check if the redirectUrl starts with "en/" or "de/" and remove it
    if (redirectUrl.startsWith('en/') || redirectUrl.startsWith('de/')) {
        modifiedUrl = redirectUrl.substring(3);
    }

    // Get the 'code' and 'state' parameters from the URL
    const urlParams = new URLSearchParams(window.location.search);
    const codeString = '?code=' + urlParams.get('code');
    const stateString = '&state=' + urlParams.get('state');
    let finalURL = modifiedUrl
        //if code or state null remove from link
    if (!codeString || !stateString) {

        
    } else{
    finalURL = modifiedUrl + codeString + stateString;
    }



    // Remove the localStorage item before redirecting
    localStorage.removeItem(POST_LOGIN_REDIRECT_URL_KEY);

    // Redirect to the modified URL with the code and state parameters
    window.location.href = finalURL;
    console.log('Redirecting to saved URL:', modifiedUrl);
}